			...
			for (int x = 0; x < width; x++)
			{
				for (int z = 0; z < ncomp; z++)
				{
					if (color_type == PNG_COLOR_TYPE_PALETTE)
					{
						...
						pixels[((height - 1 - y)*width + x) * 4 + 0] = clr.red;
						pixels[((height - 1 - y)*width + x) * 4 + 1] = clr.green;
						pixels[((height - 1 - y)*width + x) * 4 + 2] = clr.blue;
					}
					else pixels[((height - 1 - y)*width + x) * 4 + z] = *buf++;
				}
				if (ncomp == 3)
					pixels[((height - 1 - y)*width + x) * 4 + 3] = 255;
			}
		}
	}
